...
class ElasticDB:
    ...
    # Vector 유사도를 이용한 검색
    def dense_retrieve(self, embedding_module, query_str, size):
...
class OpenAI_GPT:
    def __init__(self, llm_model="gpt-3.5-turbo-1106", key_file='./ex-key/openai-helpotcreator-key-all.txt', es_db=None):
        self.llm_model = llm_model
        self.api_key = self.load_api_key(key_file)  # 키 로딩 중 오류 발생 시 예외 발생
        self.client = OpenAI()
        self.es_db = es_db  # Elasticsearch 또는 다른 검색 엔진 객체
    ...
    def answer_question(self, messages):
        ...
        if result.choices[0].message.tool_calls:
            ...
            search_result = self.es_db.sparse_retrieve(standalone_query, 3)
...
def start():
    ...
    es_db.create_es_index("test")

    # DB 인덱스에 (원본 + embeddings) 내용을 저장한다.
    ret = es_db.bulk_add("test", index_docs)
    ...
    # 역색인을 사용하는 검색 예제
    search_result_retrieve = es_db.sparse_retrieve(test_query, 3)
    ...
    search_result_retrieve = es_db.dense_retrieve(embedding_module, test_query, 3)
    ...
    react_agent = OpenAI_GPT(llm_model, openai_key_file, es_db)
